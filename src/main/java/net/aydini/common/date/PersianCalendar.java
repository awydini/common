...
public class PersianCalendar extends Calendar
{
     ...
     * 
     */
    private static final long serialVersionUID = 1137596027451645908L;
    ...
    PersianCalendar(int year, int month, int dayOfMonth, int hourOfDay, int minute, int second, int millis)
    {
        super();
    ...
    public static YearMonthDate gregorianToJalali(YearMonthDate gregorian)
    {
        ...
        gregorianDayNo = 365 * gregorian.getYear() + (int) Math.floor((gregorian.getYear() + 3) / 4)
                - (int) Math.floor((gregorian.getYear() + 99) / 100) + (int) Math.floor((gregorian.getYear() + 399) / 400);
        ...
        if (gregorian.getMonth() > 1
                && ((gregorian.getYear() % 4 == 0 && gregorian.getYear() % 100 != 0) || (gregorian.getYear() % 400 == 0)))
        {
            ++gregorianDayNo;
        }
    ...
    public static YearMonthDate jalaliToGregorian(YearMonthDate jalali)
    {
        ...
        jalaliDayNo = 365 * jalali.getYear() + (int) (jalali.getYear() / 33) * 8 + (int) Math.floor(((jalali.getYear() % 33) + 3) / 4);
    ...
    {
        switch (dayOfWeek(PersianCalendar.jalaliToGregorian(new YearMonthDate(year, 0, 1))))
        {
        case 2:
            dayOfYear++;
            break;
    ...
    public static boolean isLeepYear(int year)
    {
        ...
        if ((year % 33 == 1 || year % 33 == 5 || year % 33 == 9 || year % 33 == 13 || year % 33 == 17 || year % 33 == 22 || year % 33 == 26
                || year % 33 == 30))
        {
            return true;
        }
    ...
    public void set(int field, int value)
    {
        ...
        case DST_OFFSET:
        {
            if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) && isSet(MINUTE) && isSet(SECOND)
                    && isSet(MILLISECOND))
            {
                cal = new GregorianCalendar();
                YearMonthDate yearMonthDate = jalaliToGregorian(
